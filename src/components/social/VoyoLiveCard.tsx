...
export const VoyoLiveCard = ({ onSwitchToVOYO }: VoyoLiveCardProps = {}) => {
  ...
  const { isLoggedIn, coreId } = useUniverseStore();
  ...
  useEffect(() => {
    if (!coreId || !isLoggedIn) return;

    const loadData = async () => {
      try {
        const [friendsList, activity] = await Promise.all([
          friendsAPI.getFriends(coreId),
          activityAPI.getFriendsActivity(coreId, APP_CODES.VOYO),
        ]);
  ...
  }, [coreId, isLoggedIn, currentTrack]);
